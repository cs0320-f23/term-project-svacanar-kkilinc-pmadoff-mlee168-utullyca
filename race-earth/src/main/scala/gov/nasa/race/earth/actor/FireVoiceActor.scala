...
def handleFireMessage: Receive = {
  ...
  case BusEvent(_, wildfireData: WildfireDataAvailable, _) =>
    ...
    */
    def makeRequest(importedFireTextData: FileRetrieved): Unit = {
      ...
      if (apiAvailable) {
        ...
        warning(s"Generated JSON body for HTTP request: $bodyJson")
        ...
        httpRequestStrict(apiPort, HttpMethods.POST, entity = reqEntity) { // maybe max time out in
          case Success(strictEntity) =>
            warning(s"makeRequest: download http request payload complete:")
            ...
            self ! processResponse(data, importedFireTextData) // Effectively a synchronization thread: allows us to not care about the execution of
          case Failure(x) =>
  ...
  private def processResponse(response: String, importedFireTextData: FileRetrieved): WildfireGeolocationData = {
    // Received from the Python FV-API
    // Note: JSON is order invariant (make this handle that and can just
    val p = new StringJsonPullParser
    if (p.initialize(response)) {
      val fireTextFileOption = Option(importedFireTextData.req.file)
      ...
      p.foreachMemberInCurrentObject {
        case ConstAsciiSlice("date".getBytes) => date = p.readOptionalDateTimeMember(asc("date"))
        case ConstAsciiSlice("Incident_ID".getBytes) => incidentId = Some(asc(p.quotedValue.toString).toString)
        case ConstAsciiSlice("Call_ID".getBytes) => callId = Some(asc(p.quotedValue.toString).toString)
        case ConstAsciiSlice("Coordinates".getBytes) => coordinates = Some(List(asc(p.quotedValue.toString).toString))
        case ConstAsciiSlice("Incident_Report".getBytes) => incidentReport = Some(asc(p.quotedValue.toString).toString)
        case ConstAsciiSlice("Severity_Rating".getBytes) => severityRating = Some(asc(p.quotedValue.toString).toString)
        case ConstAsciiSlice("Coordinate_Type".getBytes) => coordinateType = Some(asc(p.quotedValue.toString).toString)
        // ... other cases if any ...
      }      // Dinner +
      WildfireGeolocationData (
        fireTextFile = fireTextFileOption,
        date = date,
      ...
      throw new RuntimeException("Failed to initialize JsonPullParser")
    }
  }
